...
function loadSourceUvContext(modelPath: string): SourceUvContext {
    if (path.extname(modelPath).toLowerCase() !== '.gltf') {
        throw new Error('Invalid model path extension');
    }
    ...
    for (const gltfBuffer of gltfBuffers) {
        ...
        if (typeof uri !== 'string' || uri.length === 0) {
            throw new Error('Invalid buffer URI');
        }
        ...
        if (!fs.existsSync(bufferPath)) {
            throw new Error(`Buffer file not found: ${bufferPath}`);
        }
...
export function applyAsoboGeometryRepair(document: Document) {
...
export function repairDocument(document: Document, modelPath: string, issuesJsonPath: string): Document {
    ...
    const sourceUvContext = loadSourceUvContext(modelPath);
        ...
        switch (issue.code) {
            ...
            case 'VALUE_NOT_IN_RANGE': {
                ...
                if (material) {
                    ...
                    if (emissiveFactor.some((value: number) => value < 0 || value > 1)) {
                    ...
                    if (baseColorFactor.some((value: number) => value < 0 || value > 1)) {
