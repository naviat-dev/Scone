...
public class SceneryConverter : INotifyPropertyChanged
{
	...
	public void ConvertScenery(string inputPath, string outputPath, bool isGltf, bool isAc3d)
	{
		...
		foreach (var kvp in modelReferencesByTile)
		{
			...
			string placementStr = $"OBJECT_STATIC {activeName} {center.Y.ToString(CultureInfo.InvariantCulture)} {center.X.ToString(CultureInfo.InvariantCulture)} {center.Z.ToString(CultureInfo.InvariantCulture)} {(270)} {0} {(90)}";
			if (hasXml)
			{
