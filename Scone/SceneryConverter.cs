...
public class SceneryConverter : INotifyPropertyChanged
{
						...
						foreach (int tile in tileIndices)
						{
							...
							if (current.modelObjects.Count > 1 || scalesTrunc.Length > 0 || current.lightObjects?.Count > 0)
							{
								...
								foreach (float scale in scales)
								{
									...
									for (int i = 0; i < current.modelObjects.Count; i++)
									{
	...
	private XmlNode CreateLodElement(XmlDocument doc, float radius, int minSize, int? maxSize)
	{
		...
		prodMin1.AppendChild(valueRadiusMin);
		prodMin1.AppendChild(propertyScreenHeightMin);
		...
		deg2radMin.AppendChild(fovPropertyMin);
		tanMinProd.AppendChild(valueHalfMin);
		tanMinProd.AppendChild(deg2radMin);
		tanMin.AppendChild(tanMinProd);
		prodMin2.AppendChild(valueMinSize);
		prodMin2.AppendChild(tanMin);

		// Append to min property div
		divMin.AppendChild(prodMin1);
		divMin.AppendChild(prodMin2);
		exprMin.AppendChild(divMin);
		minProp.AppendChild(exprMin);

		lodElem.AppendChild(minProp);
		...
		if (maxSize != null)
		{
			...
			prodMax1.AppendChild(valueRadiusMax);
			prodMax1.AppendChild(propertyScreenHeightMax);
			...
			deg2radMax.AppendChild(fovPropertyMax);
			tanProdMax.AppendChild(valueHalfMax);
			tanProdMax.AppendChild(deg2radMax);
			tanMax.AppendChild(tanProdMax);
			prodMax2.AppendChild(valueMaxSize);
			prodMax2.AppendChild(tanMax);

			// Append to max property div
			divMax.AppendChild(prodMax1);
			divMax.AppendChild(prodMax2);
			exprMax.AppendChild(divMax);
			maxProp.AppendChild(exprMax);
